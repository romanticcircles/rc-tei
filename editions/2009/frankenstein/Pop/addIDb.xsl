...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:tei="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="tei">
	...
	<xsl:template name="fileName">
		...
		<xsl:value-of select="concat('#', preceding::tei:div[@type='appPopA']/tei:list/tei:item/tei:ref[@target=$number])"/>
	</xsl:template>

	<xsl:template match="tei:div[@type='appPopA']"/>

	<xsl:template match="tei:div[@type='appPopB']/tei:list/tei:item/tei:term">
		<xsl:for-each select=".">
