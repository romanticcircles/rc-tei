...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:xhtml="http://www.w3.org/1999/xhtml"
		...
		<TEI xmlns="http://www.tei-c.org/ns/1.0">
			<teiHeader>
				<fileDesc>
					...
					<publicationStmt>
						<idno type="edition">
							<xsl:value-of select="'frank27.chars'"/>
						</idno>
					...
					<sourceDesc>
						<biblStruct>
							<analytic>
								<title level="a">
									<xsl:text>STUDY AIDS : CHARACTERS</xsl:text>
								</title>
								<author>
			...
			<text>
				<body>
					<head>STUDY AIDS : CHARACTERS</head>
	...
	<xsl:template match="xhtml:body/xhtml:table/xhtml:tr[1]"/>

	<xsl:template match="xhtml:body/xhtml:table/xhtml:tr[2]">
		<xsl:apply-templates select="xhtml:td/xhtml:div"/>
		<xsl:apply-templates select="xhtml:td/xhtml:hr"/>
		<xsl:apply-templates select="xhtml:td/xhtml:a"/>
	</xsl:template>

	<xsl:template match="xhtml:body/xhtml:table/xhtml:tr[2]/xhtml:td/xhtml:hr"/>
	<xsl:template match="xhtml:body/xhtml:table/xhtml:tr[2]/xhtml:td/xhtml:p"/>

	<xsl:template match="xhtml:body/xhtml:table/xhtml:tr[3]"/>
	...
	<xsl:template match="xhtml:body/xhtml:table/xhtml:tr[2]/xhtml:td/xhtml:div">
		<xsl:apply-templates select="xhtml:h3"/>
		<xsl:apply-templates select="xhtml:p | xhtml:blockquote | xhtml:h2 | xhtml:h4 | xhtml:ol | xhtml:li | xhtml:br"/>
	</xsl:template>

	<xsl:template match="xhtml:body/xhtml:table/xhtml:tr[2]/xhtml:td/xhtml:div/xhtml:h3">
		<xsl:variable name="myId">
			<xsl:if test="contains(., 'Beaufort')">
				<xsl:value-of select="'beaufort'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Henry')">
				<xsl:value-of select="'clerval'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Creature')">
				<xsl:value-of select="'creature'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Agatha')">
				<xsl:value-of select="'agatha'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Felix')">
				<xsl:value-of select="'felix'"/>
			</xsl:if>
			<xsl:if test="contains(., 'M. De Lacey')">
				<xsl:value-of select="'delacey'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Alphonse')">
				<xsl:value-of select="'alphonse'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Caroline')">
				<xsl:value-of select="'caroline'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Elizabeth')">
				<xsl:value-of select="'eliz'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Ernest')">
				<xsl:value-of select="'ernest'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Victor')">
				<xsl:value-of select="'frank'"/>
			</xsl:if>
			<xsl:if test="contains(., 'William')">
				<xsl:value-of select="'william'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Kirwin')">
				<xsl:value-of select="'kirwin'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Krempe')">
				<xsl:value-of select="'krempe'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Justine')">
				<xsl:value-of select="'justine'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Daniel')">
				<xsl:value-of select="'nugent'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Safie')">
				<xsl:value-of select="'safie'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Margaret')">
				<xsl:value-of select="'saville'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Waldman')">
				<xsl:value-of select="'waldman'"/>
			</xsl:if>
			<xsl:if test="contains(., 'Walton')">
				<xsl:value-of select="'walton'"/>
			</xsl:if>
		</xsl:variable>
		<term xmlns="http://www.tei-c.org/ns/1.0" rend="bold">
	...
	<xsl:template match="xhtml:body//*/xhtml:blockquote">
		<xsl:for-each select=".">
			<xsl:choose>
				<xsl:when test="preceding-sibling::xhtml:p">
					<xsl:apply-templates/>
				</xsl:when>
				<xsl:otherwise>
					<p xmlns="http://www.tei-c.org/ns/1.0">
	...
	<xsl:template match="xhtml:body//*/xhtml:ol">
		<xsl:for-each select=".">
			<div xmlns="http://www.tei-c.org/ns/1.0" type="simple">
				<xsl:apply-templates/>
			</div>
		</xsl:for-each>
	</xsl:template>
	...
	<xsl:template name="tocCharacs">
		<xsl:for-each
			select="document(/Frankenstein/index/@code)/xhtml:html/xhtml:body/xhtml:table/xhtml:tr/xhtml:td/xhtml:div[@class='appendix']/xhtml:table/xhtml:tr/xhtml:td/xhtml:a">
			<item xmlns="http://www.tei-c.org/ns/1.0">
				<ref xmlns="http://www.tei-c.org/ns/1.0">
					<xsl:attribute name="target">
						<xsl:value-of select="substring-before(substring-after(@href, 'Chars/'), '.html')"/>
					</xsl:attribute>
