...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:tei="http://www.tei-c.org/ns/1.0"
	xmlns:html="http://www.w3.org/1999/xhtml" xmlns:xhtml="http://www.w3.org/1999/xhtml"
	exclude-result-prefixes="tei rdf nines dc role dcterms rdfs html" extension-element-prefixes="redirect"
	xmlns:redirect="org.apache.xalan.xslt.extensions.Redirect" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
	xmlns:nines="http://www.nines.org/schema#" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:role="http://www.loc.gov/loc.terms/relators/"
	xmlns:dcterms="http://purl.org/dc/terms/" xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#">
	<xsl:output method="xml" encoding="utf-8" indent="yes"/>
	<xsl:strip-space elements="*"/>

	<!-- when running for rdf, leave only "tei" in "exclude-result-prefixes" above; when running for TEI, leave them all (tei rdf nines dc role
	dcterms rdfs) -->


	<xsl:param name="input" select="document('1818v1list.xml')"/>

	<xsl:template match="/">
		<xsl:for-each select="$input/Frankenstein/part">
			<xsl:variable name="name">
				<xsl:value-of select="substring-before(@code, '.html')"/>
			</xsl:variable>
			<xsl:choose>
				<xsl:when test="element-available('redirect:write')">
					<xsl:for-each select="xhtml:html">
							...
							<TEI xmlns="http://www.tei-c.org/ns/1.0">
								<teiHeader>
									<fileDesc>
										...
										<publicationStmt>
											<idno type="edition">
												<xsl:value-of select="concat('frank27.1818v1', $name)"/>
											</idno>
											...
											<availability status="restricted">
												...
												<p>Unless otherwise noted, all Pages and Resources mounted on Romantic Circles are copyrighted by the
													author/editor and may be shared only in accordance with the Fair Use provisions of U.S. copyright
													law. Except as expressly permitted by this statement, redistribution or republication in any
													medium requires express prior written consent from the author/editors and advance notification of
													Romantic Circles. Any requests for authorization should be forwarded to Romantic Circles:>
								...
								<text>
									<body>
										<xsl:apply-templates select="descendant::xhtml:div[@class='novel']/xhtml:ol/xhtml:li/xhtml:p"/>
									</body>
								</text>
							</TEI>
						</redirect:write>
